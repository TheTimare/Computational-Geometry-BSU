...
void diameterJarvis(int i) {
	...
	glutTimerFunc(msPerFrame, diameterJarvis, ++i); //basis of cycle
}
...
void gl_display() {
	...
	for (int i = 0; i < diameterPoints.size(); i++) {
		glVertex2d(diameterPoints[i].x, diameterPoints[i].y);
	}
	glEnd();
...
void glut_init(int argc, char **argv, int numberOfPoints, int limitOfPointsValue) {
	...
	glutTimerFunc(msPerFrame, diameterJarvis, 0);  //timer startup(it is recursive)
