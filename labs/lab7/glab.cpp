...
const int msPerFrame = 300; //50 for 20fps, 33 for ~30fps, 16.7 for ~60fps
...
void DynamicHullCycle(int i) {
	...
	Point newPoint = generatePoint(10);
	while (isVectorContain(points, newPoint)) {
		newPoint = generatePoint(10);
	}

	points.push_back(newPoint);
	convexHull = buildDynamicConvexHull(convexHull, points);
